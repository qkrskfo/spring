...
<body>
	...
	<!-- movieNo를 어떻게 보내야할지 모르겠습니다... -->
	<c:forEach items="${movieList}" var="movie">
		 <li>제목: <a class="title" href="reservation_summary" no="${movie.movieNo }">${movie.title}의 movieNo -> ${movie.movieNo }</a></li> 
		<form>
				  <input class="movie" type="button" name="movie" value="${movie.title}" no="${movie.movieNo }" >
				  <input type="hidden" value="${movie.movieNo }" name="movie"> 
		</form>
	</c:forEach>
	<br>
	<br>
	...
	<script src="https://code.jquery.com/jquery-3.2.1.min.js"></script>
		...
		$("a.title").click(function(e) {
			...
			location.href = "reservation_summary?movieNo=" + $(e.target).attr("no");
		});
